...
from pynamodb.attributes import LegacyUTCDateTimeAttribute
...
def _build_type_filter_condition(attribute_names, type_str):
    """
    Build a filter condition suitable for passing to scan/rate_limited_scan, which
    will filter out any items for which none of the given attributes have the given
    native DynamoDB type.
    """
    int_filter_condition = None
    for attr_name in attribute_names:
        if int_filter_condition is None:
            int_filter_condition = Path(attr_name).is_type(type_str)
        else:
            int_filter_condition |= Path(attr_name).is_type(type_str)
...
def migrate_boolean_attributes(model_class,
    ...
    for item in model_class.rate_limited_scan(_build_type_filter_condition(attribute_names, 'N'),
...
def migrate_datetime_attributes(model_class,
                                attribute_names,
                                read_capacity_to_consume_per_second=10,
                                allow_rate_limited_scan_without_consumed_capacity=False,
                                mock_conditional_update_failure=False):
    """
    Migrates UTCDateTimeDateTime attributes per GitHub `issue 259 <https://github.com/pynamodb/PynamoDB/issues/259>`_.

    Will scan through all objects and perform a conditional update
    against any items that store any of the given attribute names as
    strings. Rate limiting is performed by passing an appropriate
    value as ``read_capacity_to_consume_per_second`` (which defaults to
    something extremely conservative and slow).
    ...
    for item in model_class.rate_limited_scan(_build_type_filter_condition(attribute_names, 'S'),
                                              read_capacity_to_consume_per_second=read_capacity_to_consume_per_second,
                                              allow_rate_limited_scan_without_consumed_capacity=allow_rate_limited_scan_without_consumed_capacity):
        actions = []
        condition = None
        for attr_name in attribute_names:
            ...
            if old_value is None:
                continue
            if not isinstance(old_value, datetime):
                raise ValueError('attribute {0} does not appear to be a datetime attribute'.format(attr_name))

            actions.append(getattr(model_class, attr_name).set(getattr(item, attr_name)))

            if condition is None:
                condition = Path(attr_name) == LegacyUTCDateTimeAttribute().serialize(old_value)
            else:
                condition = condition & Path(attr_name) == LegacyUTCDateTimeAttribute().serialize(old_value)
