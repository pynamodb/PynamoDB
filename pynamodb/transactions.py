...
class TransactGet(Generic[_M], Transaction):
    ...
    async def get(self, model_cls: Type[_M], hash_key: _KeyType, range_key: Optional[_KeyType] = None) -> _ModelFuture[_M]:
        """
...
class TransactWrite(Transaction):
    ...
    async def condition_check(self, model_cls: Type[_M], hash_key: _KeyType, range_key: Optional[_KeyType] = None, condition: Optional[Condition] = None):
    ...
    async def delete(self, model: _M, condition: Optional[Condition] = None) -> None:
        operation_kwargs = await model.get_delete_kwargs_from_instance(condition=condition)
        self._delete_items.append(operation_kwargs)

    async def save(self, model: _M, condition: Optional[Condition] = None, return_values: Optional[str] = None) -> None:
        operation_kwargs = await model.get_save_kwargs_from_instance(
            condition=condition,
    ...
    async def update(self, model: _M, actions: List[Action], condition: Optional[Condition] = None, return_values: Optional[str] = None) -> None:
        operation_kwargs = await model.get_update_kwargs_from_instance(
            actions=actions,
            condition=condition,
            return_values_on_condition_failure=return_values
        )
